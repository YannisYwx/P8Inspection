package com.p8.common.widget.pickerview.utils;
...
public class ChinaDate {
    ...
    private final static String[] N_STR1 = new String[]{"", "正", "二", "三", "四",
            "五", "六", "七", "八", "九", "十", "冬", "腊"};
    ...
    private static SimpleDateFormat sdf = new SimpleDateFormat("yyyy年M月d日 EEEEE",Locale.CHINA);
    ...
    public static ArrayList<String> getMonths(int year) {
        ...
        for (int i = 1; i < N_STR1.length; i++) {
            baseMonths.add(N_STR1[i] + "月");
        }
        if (leapMonth(year) != 0) {
            baseMonths.add(leapMonth(year), "闰" + N_STR1[leapMonth(year)] + "月");
        }
        return baseMonths;
    }
